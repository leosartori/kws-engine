...
def rnn_autoencoder_model(num_tokens, num_units):
    """
    Modello di autoencoder RNN, codifica e decodifica gli spettogrammi cercando di ricostruirli.
    ...
    # N_l = 2 (due livelli)
    encoder_outputs_1, encoder_state_1 = GRU(num_units, return_sequences=True, return_state=True, dropout=0.2)(encoder_inputs) # input = [batch, timesteps, feature]
    encoder_outputs_2, encoder_state_2 = GRU(num_units, return_state=True, dropout=0.2)(encoder_outputs_1) # input = [batch, timesteps, feature]
    ...
    #     GRU(num_units, return_sequences=True, dropout=0.2))(decoder_inputs, initial_state=init_dec_1)
    # decoder_outputs_2 = Bidirectional(
    #     GRU(num_units, return_sequences=True, dropout=0.2))(decoder_outputs_1, initial_state=init_dec_2)

    # N_l = 2 (due livelli)
    decoder_outputs_1 = Bidirectional(
        GRU(num_units, return_sequences=True, dropout=0.2))(decoder_inputs, initial_state=decoder_init_state_list)
    decoder_outputs_2 = Bidirectional(
        GRU(num_units, return_sequences=True, dropout=0.2))(decoder_outputs_1, initial_state=decoder_init_state_list)
    ...
    final_output = Dense(num_tokens, activation=None)(decoder_outputs_2)
