...
num_units = 5 # GRU units in encoder and decoder       questo numero dovrebbe essere la dimensione del vettore in ouput per ogni timestep step nella RNN, e anche la dimensione del veottore di stato interno, vero?
...
# quindi hai creato un array con ripetute lo stesso file wav 10 volte vero?
...
# mi sembra giusta come idea, come ci aveva detto a lezione


# train the model
opt = tf.keras.optimizers.Adam(learning_rate=0.01)
autoenc.compile(optimizer = opt, loss = tf.keras.losses.MeanSquaredError(), metrics = ["accuracy"])
autoenc.fit(x = [X_train, X_train_shifted], y = X_train, epochs = 20, batch_size = 32) # qui diminuirei la dimensione della batch size, e intanto anche il numero di epoche
